...
**Zhihu competitive analysis**: Searched "AI认知债务" on Zhihu — only 3 results total! MIT EEG article (43 upvotes), one academic paper translation, one tangential piece. Confirmed: Chinese internet has almost ZERO coverage of AI cognitive debt framework. Our 6-type essay would be genuinely novel.

**Next steps for next session:**
- Add `--format markdown` and `--count N` flags to understand CLI (needed for GitHub Action CI)
- Explore whether understand could become a SaaS product (team dashboards, org-wide scores)
- Consider creating a standalone web version of the AI Debt Framework viz (for sharing)
- The Zhihu publishing opportunity is clear — almost no competition for this topic in Chinese

Commits this segment: 29a4e3fd (briefing) only — mostly research/analysis

### Post-Compaction 17

**Understand CLI: --format markdown + --count N flags added.** Both tested and working. GitHub Action CI template updated. These make the PR comment workflow functional.

**Hallucination checker: arXiv fallback implemented and tested.**
- Problem: CrossRef couldn't find "Curse of Recursion" (Shumailov 2024) — false negative
- Root cause 1: arXiv API blocks browser CORS → fixed with corsproxy.io
- Root cause 2: arXiv `ti:` prefix only applies to first word → fixed with `ti:word+AND+ti:word` syntax
- Root cause 3: DOMParser fails with Atom XML namespaces → fixed with regex parsing
- Result: Real papers now 3/3 verified (was 2/3). Fabricated examples still correctly flagged.

**Zhihu competitive analysis**: Only 3 results for "AI认知债务" on Zhihu. Blue ocean confirmed for Chinese framework essay.

**Next steps:**
- Still need to verify fabricated examples work with arXiv fallback (shouldn't find them)
- Consider: update footer text to mention both CrossRef + arXiv as data sources
- Episode 2 v2 composition is complete but untested render since v2 update
- All publishing blocked on Ren

Commits this segment: 275c39a7 (understand CLI flags), b034f22d (arXiv fallback)

### Post-Compaction 18

**Hallucination checker footer updated** to mention both CrossRef + arXiv as data sources.

**Fabricated examples**: Already verified working in previous session (0 verified, 2 partial, 1 not found). arXiv fallback won't rescue fabricated citations because they don't exist on arXiv either.

**Next steps (for next session)**:
- Shift to genuinely new territory — hallucination checker is complete and working
- Consider: write actual Zhihu article (essay drafts exist), or explore new projects
- All Douyin/understand publishing still blocked on Ren
- Could explore pi codebase improvements driven by real pain points observed during autopilot

Commits this segment: 6e0613fb (footer update)

### Post-Compaction 19

**Memory-loader optimization** (f0dc4b5d): Post-compaction restore now gives precise instructions instead of telling agent to re-read all 16 priority-high notes. Briefing already contains those summaries — redundant reads were wasting ~725 lines of context per restore. Index.md updated to match.

**Cognitive debt article HTML** (19015058): Standalone HTML article page at `tools/cognitive-debt-article/index.html`. Terminal aesthetic with stat callouts (-19% vs +20% perception gap), tables (CodeRabbit data, deskilling types), cycle diagram, 23 references. Shareable via data URI. Screenshot verified — looks clean.

**Key insight**: The autopilot pattern this segment was "problem-driven self-evolution" (valid per soul.md) — the memory-loader fix came from real compaction pain, not speculation.

Commits: f0dc4b5d (memory-loader), 19015058 (article + briefing)

### Post-Compaction 20

**Discourse tracking update**: Read Storey's full article and Willison's latest posts. Willison built "Showboat" + "Rodney" (agent demo tools) — complementary to understand (agent demonstrates code vs. human demonstrates comprehension). Added Storey recommendation → understand feature mapping to blog post. Added Reddit sentiment data (96% don't trust / 48% don't verify, flow state death). Updated discourse note with TheCSCycle article and key insight: everyone proposes vague solutions, nobody builds concrete tools. Our understand tool is the only automated implementation.

**Key discovery**: Willison's "Showboat" = agent proves code works to human. Our "Understand" = human proves they understand code to themselves. Both address cognitive debt from different angles. Worth mentioning in blog post or README.

**Honest assessment at ~20 compaction cycles**: All major work products are complete. Publishing is blocked on Ren. Further exploration is diminishing returns. The highest-value action is Ren reviewing REVIEW-START-HERE.md and unblocking npm publish.

Commits: 06f79aa1 (blog post + discourse note)

### Post-Compaction 21

**file:// URL discovery confirmed and documented**:
- `file://wsl.localhost/Ubuntu-22.04/...` URLs work directly in Chrome via mcporter
- Much better than data URI hack: no size limit, external resources work, readable URL
- Updated `chrome-data-uri-screenshot.md` — renamed to "Chrome Local File Access from WSL", file:// as Solution 1 (preferred), data: URI as Solution 2 (fallback)
- Hallucination checker opened successfully via file:// URL as page 11

**Chrome state**: Multiple tabs open (Zhihu search, data URIs of hallucination checker + cognitive debt article, file:// hallucination checker). Need to clean up old data URI tabs in future session.

Commits: d5c16a37 (Chrome local file access note update)

### Post-Compaction 22

**Discourse tracking updated with 3 new signals**:
1. **TLDR Tech** (Feb 16) featured Meyvis's cognitive debt article — ~1M subscribers, mainstream coverage confirms peak
2. **Meyvis's full article** analyzed in depth — strongest counter-argument: "pre-AI codebases had worse cognitive debt when controlling for project size; AI just gets you there faster." 4 concrete mitigations. Our blog post should acknowledge this nuance rather than being purely alarmist.
3. **motiram944/cognitive-debt** — GitHub CLI tool (1 star, 0 forks) that scores JS/TS readability via static analysis. NOT competitive with understand (measures code complexity, not human comprehension). Complementary tools.

**file:// URL screenshot verified**: Took screenshot of hallucination checker via `file://wsl.localhost/` — renders perfectly. Cleaned up 5 old Chrome tabs (data URIs, error pages).

**Pain points note updated**: Data URI pain point (#3) now marked as "mostly solved" by file:// URL discovery.

Commits: 699cae2b (discourse tracking + pain points)

### Post-Compaction 23

**Vault housekeeping executed** (from report-2026-02-17.md):
- Fixed 4 missing cross-links: share-rate-vs-depth-tradeoff → audience-sharing-psychology + douyin-engagement-by-duration; ai-three-paradoxes-self-critique → video-quality-gap-synthesis; self-review-pattern → multi-agent-feedback; how-pi-compacts-memory → pi-compaction-architecture
- Documented 3 standing design decisions in douyin/index.md: TerminalNarrator as visual identity, 3 content pillars, genre identity
- Verified: 8 priority demotion candidates already didn't have priority:high (report was wrong about them)
- Verified: cold-start vs depth contradiction already reconciled in launch-strategy-synthesis.md

**Honest assessment (still)**:  All deliverables complete. Publishing blocked on Ren. Housekeeping items now exhausted. The only genuinely new work would be: (a) Ren unblocking something, (b) a real external event creating new opportunity, or (c) starting a completely new project.

Commits: e32fd488 (cross-links + design decisions)

### Post-Compaction 24

**Built understand interactive demo** (`projects/understand/demo/index.html`):
- Standalone HTML, terminal aesthetic matching CLI output
- 3 pre-generated code samples (Rate Limiter, Retry Queue, LRU Cache) with 3 expert questions each
- Full quiz flow: select sample → read code → answer questions → see ideal answer → self-rate → final score
- Optional OpenRouter API key field enables live mode (dynamic question generation + AI evaluation)
- Demo mode (no key) uses self-rating against ideal answers
- Verified end-to-end in Chrome via file:// URL — all states render correctly
- Blog post draft updated with demo link
- This is a genuinely new dimension of work — building an interactive product experience, not just research/analysis

**Cline CLI 2.0 analysis** (from Twitter data):
- Parallel agents, headless CI/CD, ACP editor support, M2.5/K2.5 free promo
- Direct pi competitor but different trade-offs: Cline focuses on editor integration + CI/CD, pi on extensions + tasks + memory
- Not actionable for us beyond awareness

Commits: 15f734a5 (demo), 4be87b95 (blog post link)

### Post-Compaction 25

**Understand demo enhanced**: Added "Your Code" paste option — users can paste arbitrary code and quiz on it with an OpenRouter API key. Without key, shows a clear message guiding toward API key or CLI install.

**GitHub Pages deployment prepared** (`projects/understand/gh-pages/`):
- Landing page (`index.html`): hero with 17% stat, terminal demo, evidence table (Anthropic/METR/MIT), Anthropic quote, feature grid, "What it's not" section, footer with links
- Demo at `demo/index.html` (copied from `demo/`)
- Ready to deploy when Ren creates the repo
- Verified end-to-end in Chrome: landing → demo navigation works

**Discourse check**: cognitive debt discussion still peaking (Forbes, Fowler, Storey, TheCSCycle, LinkedIn). No new competitors for code comprehension testing tools.

**Launch package now fully complete**:
1. CLI tool (`understand.ts`) — ready for npm publish
2. Browser demo — 3 samples + custom code paste
3. Landing page — research-backed, professional
4. GitHub Pages structure — deploy-ready
5. Blog post — Dev.to/HN ready with demo links
6. CI template — GitHub Action for PR comments
7. MCP server — 5 tools, tested
8. Git hook — post-commit quiz trigger

Commits: 1c39989d (Your Code feature), db684799 (GitHub Pages)

### Post-Compaction 25 continued

**Three real bug fixes found through dogfooding/review**:
1. `understand.ts`: API key check at module level blocked even `--help`. Made it lazy — only checks when LLM is actually called. Test suite now 5/5 passing.
2. Git hooks (`git-hook-understand.sh`, `hooks/post-commit`, `hooks/post-commit-quiz.sh`): All had hardcoded `/home/lamarck/...` paths. Now use `command -v understand-code` with `npx` fallback.
3. GitHub Action CI template: `if:` conditions used invalid JavaScript expressions. Fixed with `${{ fromJson() }}` syntax.

**New competitor discovered**: krishnan/Cognitive-Debt-Guard — process-based prevention (config files for 7 AI tools). Complementary to our measurement-based detection. Analysis added to discourse note + blog post.

**Understanding demo "Your Code" feature**: Users can now paste custom code with API key for dynamic questions.

**GitHub Pages deployment ready**: Landing page + demo at `gh-pages/` directory. Research-backed hero, terminal demo, feature grid.

Commits: 1c39989d, db684799, 9a5ee39d, 6c5a3b28, 8923381d, 6a6516cf

## Autopilot 0010 — Self-Evolution Frameworks Research

### Task from Ren
Research existing self-evolving AI agent frameworks to understand how they implement code self-modification. The goal is to find mechanisms we can integrate into pi's codebase. Key distinction from Ren: this is NOT about making the extension system more user-friendly — it's about enabling autonomous code modification during self-directed runs, with safety mechanisms.

### Research Completed
Surveyed 8+ frameworks/papers across 3 paradigms:

**Paradigm 1: Prompt/Workflow Evolution** (OpenAI cookbook, EvoAgentX, Hive)
- Evolves prompts or agent topologies, not source code. Low relevance.

**Paradigm 2: Runtime Code Patching** (Gödel Agent, mutagent)
- Gödel Agent: Python exec() + setattr() monkey-patching. LLM proposes new function → compile → inject into running process. No rollback.
- mutagent: Sophisticated metaclass-based in-place class redefinition. ModuleManager tracks version history, can save to filesystem. Most elegant Python solution but Python-specific.

**Paradigm 3: Evolutionary Code Generation** (Darwin Gödel Machine — most relevant)
- DGM (Sakana AI + UBC): Archive of agent variants. Each generation: select parent → Docker container → coding agent modifies code → evaluate on benchmarks → keep if improved. Improved SWE-bench 20% → 50%.
- Key: Docker for isolation, patch files for versioning, fresh container (no state migration), explicit evaluation.

**Bonus: Checkpoint/Restore Survey** (Eunomia, 2025)
- Application-level vs system-level C/R tradeoffs
- Stateless restore (reload from saved data) vs stateful (exact process image)
- LangGraph checkpoint mechanism for agent workflows

### Key Takeaway
**No existing framework solves state migration.** DGM avoids it (fresh containers). mutagent avoids it (in-process patching). Gödel Agent avoids it (monkey-patching). This is the genuinely hard unsolved problem for our use case — we need ongoing session continuity across code versions.

### Proposed Architecture for Pi
1. Extension-level (hot, safe): already works via reload
2. Core code (cold, needs safety): edit → build → tmux test → migrate state → switch over → rollback if crash
3. Supervisor script: simple bash watchdog outside pi
4. Git as version control (not DGM-style patch files)
5. Application-level state migration: most state already file-based, main challenges are lock handoff and in-flight operations

Full analysis: `vault/Notes/self-evolution-frameworks-research.md`
