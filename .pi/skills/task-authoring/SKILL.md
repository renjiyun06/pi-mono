...
1. 检查 `feedback/` 下是否有 `.md` 文件（不包括 `archived/` 子目录）
2. 如果有，优先读取所有反馈文件
3. 针对反馈中的问题采取行动
...
将反馈写入被审查方工作目录下的 `feedback/<文件名>.md`。文件名自定，建议体现审查角度（如 `execution-review.md`、`strategy-review.md`）。只操作自己的反馈文件，不修改其他文件。如果之前的反馈已被移入 `archived/`，说明已被处理，写新的即可。
