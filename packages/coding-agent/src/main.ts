...
function forkSessionOrExit(sourcePath: string, cwd: string, sessionDir?: string): SessionManager {
	try {
		return GitSessionManager.forkFrom(sourcePath, cwd, sessionDir) as unknown as SessionManager;
	} catch (error: unknown) {
...
async function createSessionManager(
	...
	if (parsed.noSession) {
		return GitSessionManager.inMemory() as unknown as SessionManager;
	}
	...
	if (parsed.sessionRepo) {
		...
		if (!isAbsolute(repoPath)) {
			...
			const mainRepo = GitSessionManager.findMostRecentGitRepo(resolvedSessionDir);
			if (mainRepo) {
		...
		return GitSessionManager.open(repoPath) as unknown as SessionManager;
	}
